...
internal class OwnedCollectibleImageDataMapperImplTest {

    private val sut = OwnedCollectibleImageDataMapperImpl()

    @Test
    fun `EXPECT owned collectible image data to be mapped`() {
        val result = sut(
            COLLECTIBLE_DETAIL,
            AMOUNT,
            FORMATTED_AMOUNT,
    ...
    @Test
    fun `EXPECT image asset detail data to be mapped`() {
        val result = sut(
            ASSET_DETAIL,
            AMOUNT,
            FORMATTED_AMOUNT,
    ...
    fun `EXPECT asset decimal to be 0 WHEN collectible detail has no fraction decimals`() {
        ...
        val expectedOwnedCollectibleImageData = OWNED_COLLECTIBLE_IMAGE_DATA.copy(decimals = 0)
        assertEquals(expectedOwnedCollectibleImageData, result)
    }

    private companion object {
        const val FRACTION_DECIMALS = 6
        val COLLECTIBLE_DETAIL = fixtureOf<BaseCollectibleDetail.ImageCollectibleDetail>().copy(
        ...
        val OWNED_COLLECTIBLE_IMAGE_DATA = OwnedCollectibleImageData(
