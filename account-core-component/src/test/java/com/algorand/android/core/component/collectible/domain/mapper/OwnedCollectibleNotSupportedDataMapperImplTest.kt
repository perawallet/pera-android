...
internal class OwnedCollectibleNotSupportedDataMapperImplTest {

    private val sut = OwnedCollectibleNotSupportedDataMapperImpl()

    @Test
    fun `EXPECT owned collectible not supported data to be mapped`() {
        val result = sut(
            COLLECTIBLE_DETAIL,
            AMOUNT,
            FORMATTED_AMOUNT,
        ...
        Assert.assertEquals(OWNED_COLLECTIBLE_NOT_SUPPORTED_DATA, result)
    }

    @Test
    fun `EXPECT not supported asset detail data to be mapped`() {
        val result = sut(
            ASSET_DETAIL,
            AMOUNT,
            FORMATTED_AMOUNT,
    ...
    fun `EXPECT asset decimal to be 0 WHEN collectible detail has no fraction decimals`() {
        ...
        val expectedOwnedCollectibleNotSupportedData = OWNED_COLLECTIBLE_NOT_SUPPORTED_DATA.copy(decimals = 0)
        Assert.assertEquals(expectedOwnedCollectibleNotSupportedData, result)
    }

    private companion object {
        const val FRACTION_DECIMALS = 6
        val COLLECTIBLE_DETAIL = fixtureOf<BaseCollectibleDetail.NotSupportedCollectibleDetail>().copy(
        ...
        val OWNED_COLLECTIBLE_NOT_SUPPORTED_DATA = OwnedCollectibleUnsupportedData(
