...
internal class OwnedCollectibleVideoDataMapperImplTest {

    private val sut = OwnedCollectibleVideoDataMapperImpl()

    @Test
    fun `EXPECT owned collectible video data to be mapped`() {
        val result = sut(
            COLLECTIBLE_DETAIL,
            AMOUNT,
            FORMATTED_AMOUNT,
    ...
    @Test
    fun `EXPECT video asset detail data to be mapped`() {
        val result = sut(
            ASSET_DETAIL,
            AMOUNT,
            FORMATTED_AMOUNT,
    ...
    fun `EXPECT asset decimal to be 0 WHEN collectible detail has no fraction decimals`() {
        ...
        val expectedOwnedCollectibleVideoData = OWNED_COLLECTIBLE_VIDEO_DATA.copy(decimals = 0)
        assertEquals(expectedOwnedCollectibleVideoData, result)
    }

    private companion object {
        const val FRACTION_DECIMALS = 6
        val COLLECTIBLE_DETAIL = fixtureOf<BaseCollectibleDetail.VideoCollectibleDetail>().copy(
        ...
        val OWNED_COLLECTIBLE_VIDEO_DATA = OwnedCollectibleVideoData(
