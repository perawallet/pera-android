    ...
    override fun bind(item: BaseCollectibleMediaItem) {
        ...
        with(binding) {
            with(fullScreenImageView) {
                setOnClickListener { listener.onAudioMediaClick(item.previewUrl ?: item.downloadUrl) }
                isVisible = item.hasFullScreenSupport
            }
            nftMediaImageView.setOnClickListener { listener.onAudioMediaClick(item.previewUrl ?: item.downloadUrl) }
        }
    }

    companion object {
        fun create(parent: ViewGroup, listener: Listener): CollectibleAudioMediaViewHolder {
