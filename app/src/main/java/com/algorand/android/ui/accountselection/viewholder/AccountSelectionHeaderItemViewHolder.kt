...
class AccountSelectionHeaderItemViewHolder(
    ...
    fun bind(item: BaseAccountSelectionListItem.HeaderItem) {
        binding.titleTextView.setText(item.titleRes)
    }

    companion object {
        fun create(parent: ViewGroup): AccountSelectionHeaderItemViewHolder {
            val binding = ItemHeaderSimpleBinding.inflate(LayoutInflater.from(parent.context), parent, false)
            return AccountSelectionHeaderItemViewHolder(binding)
        }
    }
}
