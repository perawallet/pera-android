...
import com.algorand.android.databinding.ItemRetailBannerBinding
...
class RetailBannerViewHolder(
    private val binding: ItemRetailBannerBinding,
    ...
    override fun initActionButton(item: BaseAccountListItem.BannerItem) {
        initActionButton(item, BannerType.Retail)
    }
    ...
    companion object : BannerViewHolderCreator {
        override fun create(listener: BannerListener, parent: ViewGroup): BaseBannerViewHolder {
            val binding =
                ItemRetailBannerBinding.inflate(LayoutInflater.from(parent.context), parent, false)
            return RetailBannerViewHolder(binding, listener)
        }
    }
}
