...
class WalletConnectArbitraryDataSummaryUIBuilder @Inject constructor() : WalletConnectUIBuilder {

    fun buildArbitraryDataSummary(arbitraryData: WalletConnectArbitraryData): WalletConnectArbitraryDataSummary {
        return with(arbitraryData) {
            WalletConnectArbitraryDataSummary(
                accountName = signerAccount?.name,
                accountIconDrawablePreview = signerAccount?.accountIconDrawablePreview,
                summaryTitle = AnnotatedString(R.string.arbitrary_data)
            )
        }
    }
}
