...
class AssetFilterDisplayNFTLocalSource @Inject constructor(
    sharedPreferences: SharedPreferences
) : SharedPrefLocalSource<Boolean>(sharedPreferences) {

    override val key: String
        get() = ASSET_FILTER_DISPLAY_NFT_KEY
    ...
    override fun getDataOrNull(): Boolean? {
        return with(sharedPref) {
            if (contains(ASSET_FILTER_DISPLAY_NFT_KEY)) {
                getBoolean(ASSET_FILTER_DISPLAY_NFT_KEY, false)
            } else {
                null
            }
        }
    ...
    companion object {
        private const val ASSET_FILTER_DISPLAY_NFT_KEY = "asset_filter_display_nft_key"
    }
}
