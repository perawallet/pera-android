...
class AssetFilterZeroBalanceLocalSource @Inject constructor(
    sharedPreferences: SharedPreferences
) : SharedPrefLocalSource<Boolean>(sharedPreferences) {

    override val key: String
        get() = ASSET_FILTER_ZERO_BALANCE_KEY
    ...
    override fun getDataOrNull(): Boolean? {
        return with(sharedPref) {
            if (contains(ASSET_FILTER_ZERO_BALANCE_KEY)) {
                getBoolean(ASSET_FILTER_ZERO_BALANCE_KEY, false)
            } else {
                null
            }
        }
    ...
    companion object {
        private const val ASSET_FILTER_ZERO_BALANCE_KEY = "asset_filter_zero_balance"
    }
}
