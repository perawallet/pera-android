    ...
    suspend fun getCurrencyOptionListFlow() = flow {
        ...
        currencyRepository.getCurrencyOptionList().use(
            onSuccess = { currencyOptionList ->
                val successDataResource = DataResource.Success(currencyOptionList)
                emit(successDataResource)
            },
            onFailed = { exception, code ->
                val errorDataResource = DataResource.Error.Api<List<CurrencyOption>>(exception, code)
                emit(errorDataResource)
            }
        )
    }
}
