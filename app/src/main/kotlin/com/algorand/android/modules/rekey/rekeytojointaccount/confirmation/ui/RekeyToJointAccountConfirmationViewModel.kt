...
import com.algorand.android.modules.rekey.rekeytojointaccount.confirmation.ui.usecase.RekeyToJointAccountConfirmationPreviewUseCase
...
@HiltViewModel
class RekeyToJointAccountConfirmationViewModel @Inject constructor(
    private val previewUseCase: RekeyToJointAccountConfirmationPreviewUseCase,
    private val createRekeyTransactionData: CreateRekeyTransactionData,
    savedStateHandle: SavedStateHandle
) : BaseRekeyConfirmationViewModel() {

    private val navArgs = RekeyToJointAccountConfirmationFragmentArgs.fromSavedStateHandle(savedStateHandle)
    val accountAddress: String = navArgs.accountAddress
    val authAccountAddress: String = navArgs.authAccountAddress

    private val _previewFlow = MutableStateFlow<RekeyToJointAccountConfirmationPreview?>(null)
    ...
    fun createRekeyToJointAccountTransaction() {
    ...
    fun sendRekeyTransaction(transactionDetail: SignedTransactionDetail.RekeyOperation) {
        ...
        sendTransactionJob = viewModelScope.launch(Dispatchers.IO) {
            ...
            previewUseCase.sendRekeyToJointAccountTransaction(
    ...
    private fun initPreview() {
        viewModelScope.launch {
            val initialPreview = previewUseCase.getInitialRekeyToJointAccountConfirmationPreview(
                accountAddress = accountAddress,
                authAccountAddress = authAccountAddress
            )
            _previewFlow.value = initialPreview
            previewUseCase.updatePreviewWithTransactionFee(preview = initialPreview)
                .collectLatest { preview ->
                    _previewFlow.emit(preview)
                }
        }
