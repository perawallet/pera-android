...
class AccountHistoryResourceTitleViewHolder(
    ...
    override fun bind(item: BaseTransactionItem) {
        if (item !is BaseTransactionItem.ResourceTitleItem) return
        binding.titleTextView.setText(item.stringRes)
    }

    companion object {
        fun create(parent: ViewGroup): AccountHistoryResourceTitleViewHolder {
            val binding = ItemAccountHistoryTitleBinding.inflate(LayoutInflater.from(parent.context), parent, false)
            return AccountHistoryResourceTitleViewHolder(binding)
        }
    }
}
