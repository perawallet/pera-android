...
class RecoverRegisteredAccountsFragment : DaggerBaseFragment(0) {
        ...
        viewLifecycleOwner.lifecycleScope.launch {
            viewLifecycleOwner.repeatOnLifecycle(Lifecycle.State.STARTED) {
                viewModel.viewEvent.collectLatest { event ->
                    when (event) {
                        is ViewEvent.NavigateToHome -> navToHomeNavigation()
    ...
    fun RecoverRegisteredAccountsScreen() {
        ...
        Column(
            ...
            when (val currentState = viewState) {
                ...
                is ViewState.Content -> ContentStateContent(currentState)
                is ViewState.Error -> ErrorStateContent(currentState.message)
            }
        }
    }
    ...
    fun ErrorStateContent(error: String) {
        ErrorContentWidget(
            ...
            onClick = {
                viewModel.triggerEvent(ViewEvent.NavigateBack)
            }
        )
    ...
    fun ContentStateContent(
        ...
        Scaffold { innerPadding ->
            Column(
                ...
                PeraPrimaryButton(
                    onClick = {
                        viewModel.importSelectedAccounts()
                    },
