...
internal class BackupBatchUpsertRequestMapper @Inject constructor(
    private val upsertItemRequestMapper: BackupUpsertItemRequestMapper
) {

    fun toRequest(deviceId: DeviceId, items: List<BackupBatchUpsertInput>): BackupBatchUpsertRequest {
        return BackupBatchUpsertRequest(
            deviceId = deviceId.value,
            items = items.map(upsertItemRequestMapper::toRequest)
        )
    }
}
