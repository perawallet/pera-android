...
internal class AccountInformationErrorCacheImpl(
    ...
    override fun put(address: String) {
        val currentCache = cache.getOrNull()?.getDataOrNull()
        val newCache = currentCache?.toMutableSet() ?: mutableSetOf()
        newCache.add(address)
        cache.put(CacheResult.Success.create(newCache))
    }
