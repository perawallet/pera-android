...
internal class FetchAssetsUseCase @Inject constructor(
    private val getSelectedNodeDeviceId: GetSelectedNodeDeviceId,
    private val assetRepository: AssetRepository,
    private val errorLogger: PeraErrorLogger
) : FetchAssets {

    override suspend fun invoke(assetIds: List<Long>): PeraResult<List<Asset>> {
        val deviceId = getSelectedNodeDeviceId()
        if (deviceId.isNullOrBlank()) {
            errorLogger.logError("FetchAssetsUseCase: Device ID is null or blank.")
        }
        return assetRepository.fetchAssets(assetIds, deviceId)
    }
}
