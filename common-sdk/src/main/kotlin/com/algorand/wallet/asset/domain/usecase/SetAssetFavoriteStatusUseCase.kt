...
internal class SetAssetFavoriteStatusUseCase @Inject constructor(
    ...
    override suspend fun invoke(assetId: Long, isFavorite: Boolean): PeraResult<Unit> {
        val deviceId = getSelectedNodeDeviceId()
        return if (deviceId.isNullOrBlank()) {
            errorLogger.logError("SetAssetFavoriteStatusUseCase: Device ID is null or blank.")
            PeraResult.Error(Exception())
        } else {
            assetRepository.setFavoriteStatus(assetId, deviceId, isFavorite)
        }
    }
}
