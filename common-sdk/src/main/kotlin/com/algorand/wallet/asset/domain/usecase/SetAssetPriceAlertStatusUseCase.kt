...
internal class SetAssetPriceAlertStatusUseCase @Inject constructor(
    private val getSelectedNodeDeviceId: GetSelectedNodeDeviceId,
    private val assetRepository: AssetRepository,
    private val errorLogger: PeraErrorLogger
) : SetAssetPriceAlertStatus {

    override suspend fun invoke(assetId: Long, enabled: Boolean): PeraResult<Unit> {
        val deviceId = getSelectedNodeDeviceId()
        return if (deviceId.isNullOrBlank()) {
            errorLogger.logError("SetAssetPriceAlertStatusUseCase: Device ID is null or blank.")
            PeraResult.Error(Exception())
        } else {
            assetRepository.setPriceAlertStatus(assetId, deviceId, enabled)
        }
    }
}
