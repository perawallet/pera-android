...
internal class InitializeAllBannersUseCase @Inject constructor(
    ...
    private val initializeSpotBanners: InitializeSpotBanners
) : InitializeAllBanners {

    override suspend fun invoke(deviceId: String) {
        supervisorScope {
            val bannersDeferred = async { initializeBanners(deviceId) }
            val spotBannersDeferred = async { initializeSpotBanners(deviceId) }
            awaitAll(bannersDeferred, spotBannersDeferred)
        }
    }
}
