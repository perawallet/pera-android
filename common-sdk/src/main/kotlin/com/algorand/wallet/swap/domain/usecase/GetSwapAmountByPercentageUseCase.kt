...
internal class GetSwapAmountByPercentageUseCase @Inject constructor(
    ...
    override suspend fun invoke(payload: SwapAmountByPercentagePayload): PeraResult<BigDecimal> {
        return getSelectedSwapAssetDetail(payload.address, payload.assetInId).use(
            onSuccess = { assetDetail ->
                val percentage = payload.percentage.toBigDecimal().movePointLeft(2).toPlainString()
                swapRepository.calculateSwapAmount(payload, percentage).map { calculatedAmount ->
                    calculatedAmount.toBigDecimal().movePointLeft(assetDetail.decimal)
                }
            },
