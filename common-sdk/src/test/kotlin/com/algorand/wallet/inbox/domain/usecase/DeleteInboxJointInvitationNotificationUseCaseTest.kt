...
internal class DeleteInboxJointInvitationNotificationUseCaseTest {
    ...
    fun `EXPECT success WHEN repository succeeds`() = runTest {
        coEvery { repository.deleteInboxJointInvitationNotification(TEST_DEVICE_ID, TEST_JOINT_ADDRESS) } returns PeraResult.Success(Unit)
    ...
    fun `EXPECT error WHEN repository fails`() = runTest {
        ...
        coEvery { repository.deleteInboxJointInvitationNotification(TEST_DEVICE_ID, TEST_JOINT_ADDRESS) } returns PeraResult.Error(exception)
    ...
    fun `EXPECT correct parameters passed to repository`() = runTest {
        coEvery { repository.deleteInboxJointInvitationNotification(any(), any()) } returns PeraResult.Success(Unit)

        sut(TEST_DEVICE_ID, TEST_JOINT_ADDRESS)

        coVerify { repository.deleteInboxJointInvitationNotification(TEST_DEVICE_ID, TEST_JOINT_ADDRESS) }
    }

    @Test
    fun `EXPECT repository called with different device id`() = runTest {
        val differentDeviceId = 99999L
        coEvery { repository.deleteInboxJointInvitationNotification(differentDeviceId, TEST_JOINT_ADDRESS) } returns PeraResult.Success(Unit)

        val result = sut(differentDeviceId, TEST_JOINT_ADDRESS)

        assertTrue(result is PeraResult.Success)
        coVerify { repository.deleteInboxJointInvitationNotification(differentDeviceId, TEST_JOINT_ADDRESS) }
    }

    @Test
    fun `EXPECT repository called with different joint address`() = runTest {
        val differentAddress = "DIFFERENT_JOINT_ADDRESS"
        coEvery { repository.deleteInboxJointInvitationNotification(TEST_DEVICE_ID, differentAddress) } returns PeraResult.Success(Unit)

        val result = sut(TEST_DEVICE_ID, differentAddress)

        assertTrue(result is PeraResult.Success)
        coVerify { repository.deleteInboxJointInvitationNotification(TEST_DEVICE_ID, differentAddress) }
    }
