...
sealed class PeraResultKmp<out T : Any> {

    data class Success<out T : Any>(val data: T) : PeraResultKmp<T>()
    data class Error(val exception: Exception, val code: Int? = null) : PeraResultKmp<Nothing>()
    ...
    suspend fun <R : Any> map(transform: suspend (T) -> R): PeraResultKmp<R> {
        return when (this) {
